...
# xolo here
# these are the types of the variables by rolimons
# do not switch types with models!
...
class OwnershipData(TypedDict):
    id: int # idk why but item id gets passed around allot
    num_points: int
    timestamps: List[int]
    owners: List[int]
    bc_owners: List[int]
...
class HoardsData(TypedDict):
    num_hoards: int
    owner_ids: List[str] # never keep ids str >:( really mad at roli
    owner_names: List[str]
    quantities: List[int]

class BCCopiesData(TypedDict):
    num_bc_copies: int
    owner_ids: List[int] # now it is int :sob:
    owner_names: List[str]
    quantities: List[int]
    owner_bc_levels: List[Literal[450]]
    bc_uaids: List[str] # having a talk with roli
...
class AllCopiesData(TypedDict):
    num_copies: int
    owner_ids: List[Optional[int]] # none if user deleted
